...
//Number of iterations (HTTP requests to mimic) in test 6
$test6Iterations = 1;

//Containers to be tested (dir names)
//$containers =  ['dice','pimple', 'symfonydi', 'nette'];
$containers =  ['aura', 'dice', 'laravel',  'nette', 'njasm', 'orno', 'phalcon', 'php-di', 'pimple', 'symfonydi', 'zend-servicemanager', 'zend-di'];
...
for ($test = 1; $test <= $numTests; $test++) {
	...
	foreach ($containers as $container) {
		...
		for ($i = 0; $i < $runs; $i++) {
			$output = runScript('./test' . $test . '.php', $inis[$container]);
