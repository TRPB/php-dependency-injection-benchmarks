...
$container = new Symfony\Component\DependencyInjection\ContainerBuilder;
	...
	/*
	for ($i = 0; $i < count($classes); $i++) {
		$definition = new Symfony\Component\DependencyInjection\Definition($classes[$i], [] );
		$definition->setScope('prototype');
		$container->setDefinition($classes[$i] .'1', $definition);
	}
	
	for ($i = 0; $i < count($classes); $i++) {
		$definition = new Symfony\Component\DependencyInjection\Definition($classes[$i], [] );
		$definition->setScope('prototype');
		$container->setDefinition($classes[$i] .'2', $definition);
	}
	...
	for ($i = 0; $i < count($classes); $i++) {
		$definition = new Symfony\Component\DependencyInjection\Definition($classes[$i], [] );
		$definition->setScope('prototype');
		$container->setDefinition($classes[$i] .'3', $definition);
	}
	*/
